...
export function resolveEnding(state: WorldState): EndingDecision | null {
  ...
  // 0. 致命 flag — 最高优先级
  if (evidence.keyFlags.includes('player_captured')) {
    return {
      ending: 'deposed',
      reason: 'F1 致命：秦王被擒/控制，大势已去',
      evidence,
    };
  }
  if (evidence.keyFlags.includes('commitment_collapse')) {
    return {
      ending: 'deposed',
      reason: 'F1 致命：秦王府信心崩盘，众叛亲离',
      evidence,
    };
  }
  if (evidence.keyFlags.includes('dispatch_to_luoyang')) {
    return {
      ending: 'deposed',
      reason: 'F1 致命：秦王被遣出长安，远离权力中心',
      evidence,
    };
  }

  // 0.5 shimin_injured 降低 F1 门槛
  if (evidence.keyFlags.includes('shimin_injured') && evidence.lostOffices.length >= 1) {
    return {
      ending: 'deposed',
      reason: 'F1 即时：秦王重伤 + 官职被削，无力回天',
      evidence,
    };
  }
