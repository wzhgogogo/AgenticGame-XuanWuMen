...
| `src/engine/world/__tests__/npcAgent.test.ts` | NPC Agent（含 stance 规则、once 消耗、alertness 条件、6 NPC 决策规则） | 27 | L1 |
| `src/engine/world/__tests__/activities.test.ts` | 日常活动 | 14 | L1 |
| `src/data/skeletons/skeletons.test.ts` | 骨架模板（11 骨架结构断言 + outcome tagging + requiredNpcIds 校验） | 112 | L1 |
...
**合计 15 个文件，320 个用例**。测试文件清单变更时同步更新这张表。
...
### 7. Stance 系统（v3.4.2 更新：8 → 19）+ NPC 扩展（v3.4.4：3 → 6 NPC）+ alertness 条件（v3.4.5）
...
- `NpcDecisionRule.conditions` 支持 `patienceAbove/Below`、`alertnessAbove/Below`、`pressureAbove/Below`、`daysSinceLastActionAbove`、`relationshipAbove/Below`
- v3.4.5 新增 `alertnessAbove/Below` 条件：事件后 alertness 升高会解锁更积极的 stance
- 6 个 NPC 各有一条 `alertnessAbove: 30` 规则（id 格式 `xxx_alert`）
...
### 14. requiredNpcIds 与 activeNpcIds 合并
v3.4.5 新增 `EventSkeleton.requiredNpcIds?: string[]`，eventGenerator 会将锁定 NPC 与 LLM 选择合并去重。测试 eventGenerator 时注意：返回的 `activeNpcIds` 一定包含 `requiredNpcIds` 中 active 的 NPC。骨架结构测试中 `requiredNpcIds` 的每个 ID 必须是 6 个合法 NPC ID 之一。
