---
name: eval
description: 评估 LLM 输出质量与游戏体验——读取自动跑局日志，计算量化指标 + 涌现质量 + 规则检测 + 游戏体验指标，Claude Code 手动抽样替代 Gemini
---
...
4. 向用户报告关键发现（**P1 问题优先**），并把结论追加到 [`docs/devlog.md`](../../../docs/devlog.md)（顶部、时间倒序，加 `[eval]` 标签）
...
### Part 5: 游戏体验指标（v3.4.5.1 新增）

| 指标 | 说明 | 健康阈值 |
|------|------|----------|
| Outcome 分布 | chosenOutcome 在 success/partial/failure/disaster 中的分布，按骨架分组 | 不全是单一 outcome |
| 压力轴趋势 | 每轴的 avg daily delta / max spike / max drop / 趋势方向 / 撞顶归零天数 | 无轴长期撞顶(≥95)或归零(≤5) |
| 结局路径分析 | 距离 E1/E3/F1/F5/N1 各结局的满足/缺少条件列表 | 至少 1 个结局可达 |
| NPC 存活状态 | 6 个 NPC 的首次/末次出现日、是否消失 | 核心三人 ≥2 active |
| 骨架覆盖率 | 触发骨架数 / 11 总骨架，列出未触发骨架 | ≥ 50% |
| 对话回应度 | 有实质性玩家输入时 LLM 是否在 response 中引用相关关键词 | ≥ 50%（autoplay 下偏低是正常的，该指标更适合真人玩家日志） |
...
### Outcome 分布（v3.4.5.1 新增）
- **健康**：success/partial 为主体（60-80%），failure/disaster 偶尔出现（10-30%）
- **异常**：100% success（当前实际情况）→ LLM 倾向于给好结果，需检查骨架 resolutionSignals 是否对 failure/disaster 有足够引导，或 prompt 中缺少失败引导词
- **按骨架分布**：如果某骨架 100% failure，可能该骨架的 success 条件设计过难

### 压力轴趋势（v3.4.5.1 新增）
- **健康**：多数轴震荡上升（5-7 天有回落），1-2 轴保持相对稳定
- **异常**：轴长期撞顶（≥95连续 5+ 天）→ ceiling 或衰减率需调整；轴归零 → baseline/floor 设置有问题
- **诊断**：max spike 过大（>15）→ 某个 NPC 行为或事件 outcome 的压力修正值过高

### 骨架覆盖率（v3.4.5.1 新增）
- **健康**：≥ 50%（30 天内至少 6 种骨架被触发）
- **异常**：< 50% → 部分骨架的 precondition 阈值过高、strategy 过于单一、或 flag 前置条件未满足（如 seize_military_command 需要 tujue_invasion flag，需 70+ 天才种入）
- **诊断**：对照 `src/data/skeletons/*.ts` 的 precondition 检查每个未触发骨架的条件是否在 30 天窗口内可达
