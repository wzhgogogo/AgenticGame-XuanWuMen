...
description: 代码改动完成后的收尾验证——按改动范围跑对应验证（tsc、单测、冒烟），并追加条目到 docs/devlog.md。触发时机：改完 src/engine/、src/data/、src/types/ 下 .ts 的任意非 trivial 修改，或用户要求"走一遍验证"。
---
...
| 改动范围 | tsc | 单测 | 冒烟 | devlog | 同步 skill |
|---|---|---|---|---|---|
| 纯类型/工具函数 | ✅ | ✅ | — | 一句话 | — |
| engine/world 业务逻辑 | ✅ | ✅ | — | 详写 `[eval]` | 检查 test skill |
| promptBuilder / LLM 调用 / 解析 | ✅ | ✅ | ✅ | 详写 `[eval]` | 检查 test + eval skill |
| 规则/骨架/数据层 | ✅ | ✅ | — | 简写 | — |
| 新增或改写单测文件 | ✅ | ✅ | — | 一句话 + `[test-bug]` 若发现坑 | **必须更新 test skill 的文件清单** |
...
| **devlog.md** (docs/) | 每次非 trivial 改动 | 按日期倒序追加条目，写「为什么」和「影响」。eval 相关加 `[eval]` 标签，测试坑加 `[test-bug]` 标签。含测试表、发现的坑、层级表更新 |
...
2. **改了测试？** → test skill 文件清单 + devlog.md（加 `[test-bug]` 标签）
3. **改了 engine/prompt？** → devlog.md（加 `[eval]` 标签）+ README 开发日志
4. **以上都不是？** → 大概率不需要更新文档
...
- 不要在 devlog 里堆垃圾信息——"改了个 typo" 这种不配有条目
