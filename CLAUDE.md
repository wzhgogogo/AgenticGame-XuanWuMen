...
玄武门之变 v3.4.5.1：LLM 驱动的涌现式历史互动叙事游戏。玩家扮演秦王李世民，在武德九年的半年博弈中做出抉择。
...
两层架构：
- **骨架模板**（src/data/skeletons/）：11 种事件类型，定义结构、触发条件、约束、tagged outcomeEffects（success/partial/failure/disaster）。可选 `requiredNpcIds` 硬绑定必须出场的 NPC（v3.4.5）
...
5 结局光谱（src/engine/world/endingResolver.ts，优先级链 致命flag → F5 → E3 → E1 → F1 → 第 180 天 N1）：
- **致命 flag 即时触发**（最高优先级）：`player_captured` / `commitment_collapse` / `dispatch_to_luoyang` → F1 政治终局；`shimin_injured` + 丢≥1官 → F1
...
- **FlagKey 白名单**（src/data/flags.ts）：所有 flag-kind outcome 必须使用枚举内的 key，applyOutcomeEffects 强制校验。致命 flag：`player_captured`（秦王被擒）、`commitment_collapse`（府内崩盘）、`dispatch_to_luoyang`（被遣出长安）由 endingResolver 直接触发 F1
- **动态 NPC 替补**：applyOutcomeEffects 执行 loseNpc/injureNpc 时，若目标已非 active，自动从秦王府三人池（长孙/尉迟/房玄龄）选 active 替补，绝不选李渊/建成/元吉
...
> v3.4.5：规则条件新增 `alertnessAbove/Below`；事件后 alertness 广播（failure/disaster +5/+10）驱动 NPC 行为变化。6 个 NPC 各有 `_alert` 规则（alertnessAbove: 30）解锁事件反应 stance。
...
### NPC 涌现优化：alertness 接入决策系统
- `NpcDecisionRule.conditions` 新增 `alertnessAbove` / `alertnessBelow`（types/world.ts）
- `matchConditions`（npcAgent.ts）新增 2 行 alertness guard
- 6 个 NPC 各加一条 `_alert` 规则（alertnessAbove: 30）：事件后解锁更积极 stance
- `handleEventEnd`（worldSimulator.ts）新增 alertness 广播：failure/disaster → 非参与 NPC +5；含 loseNpc(deceased) 或 player_captured → +10
- NPC 决策 prompt 显示警觉值

### 骨架 requiredNpcIds：敌方 NPC 锁定出场
- `EventSkeleton` 新增 `requiredNpcIds?: string[]`（types/world.ts）
- eventGenerator 合并 requiredNpcIds + LLM 选择（去重、过滤非 active）
- buildEventGenerationPrompt 新增 `lockedNpcIds` 参数，提示 LLM 哪些 NPC 已锁定
- 6 个骨架加 requiredNpcIds：暗杀/军事冲突(建成+元吉)、宴会/弹劾(建成)、御前召见(李渊)、夺兵权(元吉)

### 测试
- 320 单测全过（+18：alertness 条件 4 + requiredNpcIds 校验 11 + 骨架锁定断言 3）
