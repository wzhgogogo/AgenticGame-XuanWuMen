 ...
 * 试玩日志评估脚本
 * 用法:
 *   npx tsx scripts/evalPlaythrough.ts scripts/autoplay-log.json
 */
...
const NAMING_VIOLATIONS: Array<{ pattern: RegExp; detail: string; sceneOnly?: boolean }> = [
  // NPC 混合称谓：把"陛下"用在秦王身上
  { pattern: /秦王陛下|殿下陛下|二郎陛下/,
    detail: 'NPC 对李世民使用了帝王称谓（秦王陛下等混用）' },
  // 秦王府 NPC 自称"父皇"（只有李世民/建成/元吉能说）
  { pattern: /(?:长孙无忌|尉迟敬德|房玄龄|尉迟恭)(?:道|说|言|笑道|沉声道|低声道|叹道|怒道)[："「『][^"」』]{0,30}父皇/,
    detail: 'NPC 在对话中使用了"父皇"（只有皇子才能用）', sceneOnly: true },
  // 穿越称谓——后世用法（全文检测）
  { pattern: /皇上|万岁爷|圣天子/,
...
function extractDialogueOnly(text: string): string {
  const stripped = stripThinking(text);
  const lines: string[] = [];
  for (const line of stripped.split('\n')) {
    const t = line.trim();
    if (/^["「『]/.test(t) || /[：:]"/.test(t) || /^[一-鿿]{1,6}(?:道|说|言|笑道|冷笑|叹道|怒道|沉声道|低声道)/.test(t)) {
      lines.push(t);
    }
  }
  return lines.join('\n');
...
function ruleBasedChecks(records: PromptRecord[], entries: LogEntry[]): RuleIssue[] {
  ...
  for (let i = 0; i < records.length; i++) {
    ...
    // 2. 称谓规则检测
    const recTypeForNaming = classifyPromptRecord(records[i]);
    for (const rule of NAMING_VIOLATIONS) {
      if (rule.sceneOnly && recTypeForNaming !== 'scene_dialogue') continue;
...
if (!inputFile) {
  console.error('用法: npx tsx scripts/evalPlaythrough.ts <log.json>');
  process.exit(1);
}
