/**
 * LLM 集成测试 — CLI 执行
 * 用法: npx tsx scripts/llmIntegrationTest.ts
...
await runTest('NPC 决策（长孙无忌 stance intent）', async () => {
  ...
  const stances: NpcStance[] = ['observe', 'analyze', 'advise', 'scheme'];
...
await runTest('记忆提取（memoryExtractor）', async () => {
  ...
  assert(typeof result.memories === 'object', '应有 memories 字段');
  const totalMemories = Object.values(result.memories).flat().length;
  assert(totalMemories > 0, '应提取出至少 1 条记忆');
  for (const [charId, memories] of Object.entries(result.memories)) {
